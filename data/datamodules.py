...
class GeometricDM(SmolDM):
    ...
    def _batch_to_dict(self, smol_batch):
        ...
        # print(batch.coords.shape,batch.coords.dtype,batch.coords.float().dtype)
        # print(batch.atomics.shape,batch.atomics.dtype,batch.atomics.float().dtype)
        # print(batch.adjacency.shape,batch.adjacency.dtype,batch.adjacency.float().dtype)
        # print(batch.charges.shape,batch.charges.dtype,batch.charges.long().dtype)
        # print(batch.mask.shape,batch.mask.dtype,batch.mask.long().dtype)
        ...
        coords = batch.coords[1:]
        atomics = batch.atomics[1:]
        bonds = batch.adjacency[1:]
        charges = batch.charges[1:]
        mask = batch.mask.long()[1:]
