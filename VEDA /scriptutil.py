...
def init_metrics(data_path, model, no_novelty=False):
    # Load the train data separately from the DM, just to access the list of train SMILES


    if not no_novelty:
        test_path = Path(data_path) / "test.smol"
        test_dataset = GeometricDataset.load(test_path)
        
        test_smiles = [mol.str_id for mol in test_dataset]
        print(len(test_smiles))
        test_mols = model.builder.mols_from_smiles(test_smiles, explicit_hs=True)
        test_mols = [mol for mol in test_mols if mol is not None]
        test_mols_rdkit = [mol.to_rdkit(model.builder.vocab) for mol in test_dataset]
    ...
    if not no_novelty:
        metrics['za-fcd-score'] = Metrics.FCD_Score(test_mols)
        metrics["za-novelty"] = Metrics.Novelty(train_mols)
        metrics["zb-bond-type-specific-distribution"] = Metrics.BondLengthTypeSpecificDistribution(test_mols_rdkit)
        metrics["zb-bond-angle-specific-distribution"] = Metrics.BondAngleTypeSpecificDistribution(test_mols_rdkit)
        metrics["zb-torsion-specific-distribution"] = Metrics.TorsionAngleTypeSpecificDistribution(test_mols_rdkit)
        metrics["zc-molecular-shape-distribution"] = Metrics.MolecularShapeDistribution(test_mols_rdkit)
        metrics["zc-hydrogen-bonding-geometry"] = Metrics.HydrogenBondingGeometry(test_mols_rdkit)
        metrics["zc-improved-torsion-distribution"] = Metrics.ImprovedTorsionAngleDistribution(test_mols_rdkit)
    # # metrics["zd-xtb-energy"] = Metrics.XTBEnergy()
    # metrics["zd-QED"] = Metrics.QED_Score()
    # metrics["zd-SA"] = Metrics.SA_Score()
    # metrics["zd-LogP"] = Metrics.LogP_Score()
    # metrics["zd-Lipinski"] = Metrics.Lipinski_Score()
        print('length of train smiles', len(train_smiles), 'length of test smiles', len(test_smiles),train_smiles[0],test_smiles[0])
...
def generate_molecules(model, dm, steps, stabilities=False):
    ...
    for batch in tqdm(test_dl):
        ...
        output = cuda_model._generate(batch, steps)
